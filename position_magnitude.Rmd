...
index_terms: "Cognition, framing effects, chart design, axis range, magnitude judgements."
...
([@borges_common_1974]) and rated satisfaction with the same salary as
higher when it appeared in the upper end of a range, compared to the
lower end [@brown_does_2008]. 
...
is judged as stronger [@cleveland_variables_1982]. In bar charts,
...
@correll_truncating_2020 found no reduction in effect size judgments
...
possible values can conceal important differences [@witt_graph_2019].
Based on participants' judgments of effect size, @witt_graph_2019 found
...
@yang_truncating_2021 improves on the design of previous studies which
employed only a few observations per condition [@pandey_how_2015] or
very small sample sizes [@witt_graph_2019]. Participants' ratings of
the difference between two bars consistently provided evidence of the
exaggerating effects of y-axis truncation. @yang_truncating_2021 noted
...
influenced by reference points [@tversky_judgment_1974]. Another
potential explanation discussed draws upon Grice's cooperative principle
[@grice_logic_1975]. According to this account of effective
...
inspection [@yang_truncating_2021].

```{r senators-chart, fig.cap= "In this chart, the y-axis limit is the largest possible value, rather than the largest observed value, so plotted values appear to have particularly small magnitudes.", echo=FALSE, out.width="250px", message=FALSE}
...
relationship between height and value [@gattis_structure_2002]. Both the
...
visualization [@tversky_cognitive_1997].
...
lead to misinterpretations [@okan_when_2012; @pandey_how_2015;
@woodin_conceptual_2022]. Furthermore, when a company's financial
...
appeared above the maximum value [@taylor_misleading_1986].
@sandman_high_1994 investigated assessments of magnitude in risk
...
the evidence is not substantial. @taylor_misleading_1986 did not
disclose how judgments were elicited, or provide details of their sample
size. @sandman_high_1994 only explored responses to one specific risk
...
[@elting_influence_1999; @feldman-stewart_perception_2000;
@keller_effect_2009; @okan_probability_2020;
@zikmund-fisher_whats_2005].
...
[@harris_estimating_2009; @harris_communicating_2011]. In a similar
manner, an event is associated with more substantial consequences when
it is described as more likely [@kupor_probable_2020]. One account
...
event's cause (probability-outcome correspondence principle; [
@keren_probabilityoutcome_2001]. According to this account,
...
continuous options; [@sung_visual_2018]) prevents participants from
...
[@okan_how_2016; @okan_when_2012].

### Open Research Statement

This research has been conducted following the principles of open and reproducible research. All data and analysis code is available at -github link-. This link also provides all necessary resources for running a Docker container, within which the computational environment used for analysis is recreated, meaning a fully-reproducible version of the present report can be generated.

All experiments in this paper were pre-registered (https://osf.io/qn46s/). There are no diversions from pre-registered experimental designs, exclusion criteria
...
the *buildmer* package in R [@voeten_buildmer_2022], which subsequently removed terms which did
...
at lower positions. Experimental code and a link to run the experiment are available at https://gitlab.pavlovia.org/ExPrag_UoM/riskE1.
...
The experiment was programmed in PsychoPy (version 2021.1.4; [@peirce_psychopy2_2019]) and hosted on
...
[@garcia-retamero_measuring_2016]. They were reminded that the
...
[@charalambides_we_2021], however, data for this experiment were
...
Analyses were conducted using R (version 4.1.2, [@r_core_team_r_2021]). Raw
...
linear regression [@liddell_analyzing_2018]. Throughout this paper, we
construct cumulative link mixed-effects models, using the *ordinal*
package (version 2019.12-10, [@christensen_ordinalregression_2019]) to analyse Likert scale
ratings.

Selection of random effects structures for models was automated using
the *buildmer* package (version 2.3, [@voeten_buildmer_2022]). The maximal random
...
[@barr_random_2013]. From this formula, *buildmer* initially identified
...
```{r r1-c-summary, echo=FALSE, include=FALSE}
...
position as a fixed effect ($\chi^2$(`r in_paren(r1_c.df)`) =
`r printnum(r1_c.LR)`, p `r printp(r1_c.p, add_equals = TRUE)`). Data
...
`r printnum(abs(r1_c.statistic))`, p
`r printp(r1_c.p.value, add_equals = TRUE)`). This model employed random
intercepts for each scenario and each participant. Estimated marginal
means, calculated using the *emmeans* package (version 1.7.0, [@lenth_emmeans_2021] for these ratings are plotted in Figure @ref(fig:r1-c-emm-plot).
...
```{r r1-s-summary, echo=FALSE, include=FALSE}
# summary output for E1 severity rating model
summary_extract(r1_s, "condition1")
```
...
($\chi^2$(`r in_paren(r1_s.df)`) = `r printnum(r1_s.LR)`, p
`r printp(r1_s.p, add_equals = TRUE)`). The severity of consequences was
...
`r printnum(abs(r1_s.statistic))`, p
`r printp(r1_s.p.value, add_equals = TRUE)`). This model employed random
...
points themselves (z = `r printnum(abs(r1_cl.statistic))`, p
`r printp(r1_cl.p.value, add_equals = TRUE)`) or severity of
consequences (z = `r printnum(abs(r1_sl.statistic))`, p
...
magnitudes [@kupor_probable_2020]. However, it is unclear whether the
...
vertical positions denote greater magnitudes [@tversky_cognitive_1997].
...
(@pandey_how_2015; @woodin_conceptual_2022). In E2, we provide explicit
...
to conventional charts). Experimental code and a link to run the experiment are available at https://gitlab.pavlovia.org/ExPrag_UoM/riskE2.
...
```{r r2-c-summary, echo=FALSE, include=FALSE}
# summary output for E2 chance (magnitude) rating model
summary_extract(r2_c, "pos1:ori1")
```
...
effect ($\chi^2$(`r in_paren(r2_c.df)`) = `r printnum(r2_c.LR)`, p
`r printp(r2_c.p, add_equals = TRUE)`). There was a significant
interaction between physical position and y-axis orientation (z =
`r printnum(abs(r2_c.statistic))`, p
`r printp(r2_c.p.value, add_equals = TRUE)`). This
...
graph literacy: z = `r printnum(abs(r2_cl.statistic))`, p
`r printp(r2_cl.p.value, add_equals = TRUE)`, and when controlling for
list number: z = `r printnum(abs(r2_clist.statistic))`, p
...
```{r r2-s-summary, echo=FALSE, include=FALSE}
# summary output for E2 severity rating model
summary_extract(r2_s, "ori1:pos1")
```
...
and axis orientation as a fixed effect explains significantly more
variability in ratings than a model without this interaction as a fixed
effect ($\chi^2$(`r in_paren(r2_s.df)`) = `r printnum(r2_s.LR)`, p
`r printp(r2_s.p, add_equals = TRUE)`).There was a significant
interaction between physical position and y-axis orientation (z =
`r printnum(abs(r2_s.statistic))`, p
`r printp(r2_s.p.value, add_equals = TRUE)`). This
...
(`r printnum(r2_s_contrast.c.z.ratio)`, p
...
`r printnum(abs(r2_sl.statistic))`, p
`r printp(r2_sl.p.value, add_equals = TRUE)`, and when controlling for
list number: z = `r printnum(abs(r2_slist.statistic))`, p
...
positions (the reverse of the finding for conventional charts). Experimental code and a link to run the experiment are available at https://gitlab.pavlovia.org/ExPrag_UoM/riskE3.
...
```{r r3-c-summary, echo=FALSE, include=FALSE}
# summary output for E3 chance (magnitude) rating model
summary_extract(r3_c, "condition1")
```
...
`r printp(r3_c.p, add_equals = TRUE)`). Data points' magnitudes were
...
physical positions (z = `r printnum(abs(r3_c.statistic))`, p
`r printp(r3_c.p.value, add_equals = TRUE)`). This model employed random intercepts for each
scenario. This effect remained when adjusting for participants' graph
literacy scores (z = `r printnum(abs(r3_cl.statistic))`, p
`r printp(r3_cl.p.value, add_equals = TRUE)`). Estimated marginal means
...
without this fixed effect ($\chi^2$(`r in_paren(r3_s.df)`) =
`r printnum(r3_s.LR)`, p `r printp(r3_s.p, add_equals = TRUE)`). This
...
`r printnum(abs(r3_sl.statistic))`, p
...
that y-axis limits affect *comparisons* of plotted values
[@correll_truncating_2020; @witt_graph_2019; @yang_truncating_2021], the
...
its magnitude [@sandman_high_1994]. The present set of experiments
...
[@harris_communicating_2011; @harris_estimating_2009] and that outcome
magnitude estimates change as a function of event probability
[@kupor_probable_2020]. However, whereas prior research focused on the
...
of axis range on interpretations. @yang_truncating_2021 also observed
...
receipt of elementary instruction [@okan_how_2016]. Therefore, it is
...
in presentation format [@yang_truncating_2021].
...
present appropriate values. Like @correll_truncating_2020, we
...
current objectives, *etc.*. Our findings are also relevant for assessing
...
data impacts discrimination ability [@witt_graph_2019], and may distract
...
When following Witt's [@witt_graph_2019] suggestions, data points'
...
setting axes which provide context for *individual* magnitudes, is not
...
differences discussed by @correll_truncating_2020, and reported to
benefit users by @ritchie_lie_2019. Its suitability for conveying
...
@woodin_conceptual_2022 found that comprehension is facilitated when the
...
developers [@garcia-retamero_measuring_2016], but is not the most
appropriate method [@liddell_analyzing_2018].
